...
from .calibration import from_two_points
from .pipeline import Work, auto_detect
...
class Editor:
    ...
    def save(self) -> None:
        ...
        state_path = self.out_dir / f"{self.stem}_estado.json"
        cv2.imwrite(str(img_path), annotated)
        ex.save_csv(self.seedlings, csv_path)
        ex.save_state(self.seedlings, self.px_per_cm, self.work.scale, state_path)
